...
class SynfireChain:
    def __init__(self, w, w_ab, w_ac, w_ex, w_inhibit, p_ab, p_ac, p_bbin, p_ccin, r_bin, r_cin, json_name):
        self.w = w
        self.w_ab = w_ab
        self.w_ac = w_ac
        self.w_ex = w_ex
...
#    def save_strong_synapse_mapping(self, synapse_group, filename):
#        # Ensure w and w_strong are correctly interpreted within the Brian2 context
#        synapse_strengths = (synapse_group.weight >= (self.w_strong/1000)) * 1  # This will create an array of 0s and 1s
#
#        # Create the mapping based on interpreted strengths
#        strong_synapse_mapping = [{"pre": int(i), "post": int(j), "strength": "strong" if strength else "weak"}
#                                  for i, j, strength in zip(synapse_group.i[:], synapse_group.j[:], synapse_strengths)]
#        # Save the mapping to a file
#        with open(os.path.join(f"{RES_DIR_NAME}/{self.json_name}", filename), 'w') as f:
#            json.dump(strong_synapse_mapping, f)

    #def save_strong_synapse_mapping(self, synapse_group, filename):
     #   strong_synapse_mapping = [{"pre": i, "post": j, "strength": "strong" if synapse_group.w[k] >= self.w_strong else "weak"}
      #                        for k, (i, j) in enumerate(zip(synapse_group.i, synapse_group.j))]
       # with open(os.path.join(f"{RES_DIR_NAME}/{self.json_name}", filename), 'w') as f:
        #    json.dump(strong_synapse_mapping, f)
    ...
    def run_simulation(self, json_name):
        ...
        for s in range(SIMULATIONS_NUM):
            ...
            # Inter synapses
            # Adjust the model to include 'w'
            AB = Synapses(A, B, on_pre='v_post += w_ab', delay=5 * ms)
            AB.connect(i=list(range(N - WIDTH, N)), j=list(range(WIDTH)), p= self.p_ab)

            AC = Synapses(A, C, on_pre='v_post += w_ac', delay=5 * ms)
            AC.connect(i=list(range(N - WIDTH, N)), j=list(range(WIDTH)), p=self.p_ac)

#            # Probabilistically set synapse strength and create a mapping
#            # For AB
#            strong_connections_AB = np.random.rand(len(AB)) < self.p_ab
#            AB.weight = np.where(strong_connections_AB, self.w_strong, self.w_inter) * mV
#
#            # For AC
#            strong_connections_AC = np.random.rand(len(AC)) < self.p_ac
#            AC.weight = np.where(strong_connections_AC, self.w_strong, self.w_inter) * mV
...
#'            # Save the strong synapse mappings for each relevant synapse group
#'            self.save_strong_synapse_mapping(AB, f'AB_strong_synapse_mapping_{json_name}_sim{s}.json')
#'            self.save_strong_synapse_mapping(AC, f'AC_strong_synapse_mapping_{json_name}_sim{s}.json')
    ...
    def create_pie_plot(self, json_name):
        ...
        # saves results (4 values for each pair of (w_ab, w_ac)
        data = {"w_ab": self.w_ab, "w_ac": self.w_ac, "vals": results}
...
def main_func_per_json_file(json_name):
    ...
    print(json_name)
    print(params)
    chain = SynfireChain(json_name=json_name, **{k: v * mV if k.startswith("w") else v for k, v in params.items()})
