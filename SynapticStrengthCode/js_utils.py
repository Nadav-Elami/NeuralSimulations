...
# files names
JSONS_DIR_NAME = "jsons_bbin_0.25_w1_w2"
RES_DIR_NAME = "results_bbin_0.25_w1_w2"
ALL_RESULTS_FN = "results_w1_w2_bbin_0.25.json"
...
def create_mat(file_path, col):
    ...
    for i, w1 in enumerate(np.round(np.linspace(0, 10.5, 51), 2)): #strong synaptic weight = 10.5
        for j, w2 in enumerate(np.round(np.linspace(0, 10.5, 51), 2)):
            val = [item["vals"][f"{col}"] for item in data if item.get("w_ab") == w1 and item.get("w_ac") == w2]
            if val:
                mat[i, j] = val[0]
    return mat
...
def create_heatmap(data, x_axis, y_axis, title, path):
    ...
    plt.yticks(np.arange(0, len(data), 2), np.round(np.linspace(0, 10.5, 26), 2))
    plt.xticks(np.arange(0, len(data), 2), np.round(np.linspace(0, 10.5, 26), 2))
...
def create_rgb_plot(r_mat, g_mat, b_mat, title, x_axis, y_axis, path):
    ...
    plt.title(title)
    plt.yticks(np.arange(0, len(r_mat), 2), np.round(np.linspace(0, 10.5, 26), 2))
    plt.xticks(np.arange(0, len(r_mat), 2), np.round(np.linspace(0, 10.5, 26), 2))
    plt.xlabel(x_axis)
    plt.ylabel(y_axis)
